...
Фильтр/мидлваре - конвейер обработки запроса и ответа. Любой из элементов конвейера может самостоятельно ответить на запрос и не пустить его дальше. Жизненный цикл: создаётся при запуске сервера, имеет метод InvokeAsync  
...
Основывается на HTTP. Практически расширение HTTP. Чаще всего применяется для того, чтобы достучаться к облачным хранилищам (OneDrive, Google Disk). Предполагает, что на стороне сервера есть файловый сервер (такой сервер, который притворяется файловой системой (он представляет данные в виде папок, данных, может перемещать файлы между папками, создавать, удалять файлы и т.д.)). Протокол замечателен тем, что он притворяется HTTP-протоколом, а все маршрутизаторы пропускают HTTP.  
У него расширенные методы (команды) (get, put, delete). Протокол расширялся медленно, поэтому он в нескольких RFC  

10.04.2024  
REST - просто тип интерфейса, архитектурный стиль  
Если RPC - это удалённый объект с методами, которые можно вызывать, то REST - набор URI и правила их вызова  
Обычно REST ограничивается 4 основными методами HTTP  
Restful - значит, что применяются все правила (а правил нет, они неписаные)  
Важный вопрос - как описать интерфейс для пользователя  
REST - удалённый ресурс, представляющий собой удалённую коллекцию данных. Есть подмножество элементов этой коллекции (вся, часть, один, ...)  
HATEOS - принципе формата, который закладывается с форматом запроса и ответа, в котором в ответы закладываются ссылки на другие ресурсы сервера, что позволяет отвязаться от запросов (?)  
Общепринятые правила:  
- Если делается какой-то срез интерфейса, у них должен быть общий префикс (api.belstu.by, например)  
- две сущности: коллекция и её элемент
- иерархическая связь: /api/users/123/cars/567
- ограничение количества HTTP-статусов, отправка вместе с статусом ошибки ссылку на страницу с пояснениями
- подавление статуса ответа при помощи отдельного параметра в URI для отладки кода
- версионность апи

17.04.2024  
JSON RPC - удалённый сервер предсатавляется как объект с набором методов. У метода есть имя, параметры, возвращаемый тип  
Интерфейс заключается в том, что происходит вызов удалённого метода с параметрами, получается ответ  
HTTP является транспортом  

24.04.2024  

- Nginx
  - сервер, разработанный  каким-то русским
  - архитектурно правильно разделить скрипт клиента от сервера, на котором находится сервис, обрабатывающий REST-запросы
  - для размещения js, стилей и статических ресурсов делается свой сервер
  - используется в основном для проксирования, сокрытия ip-адресов, кеширования, зеркалирования
  - есть прямое (скрывает клиента) и обратное (скрывает сервер, когда прокси-сервер прокидывает запросы клиентов снаружи во внутреннюю сеть обычным серверам) проксирования
  - поддерживает HTTP2 с приоретизацией на основе isock (?)
  - применяется для промежуточного сервера, на котором устанавливается всё касающееся HTTPS
- WebRTC (web real time communication) - набор технологий, предназначенный для потоковой передачи данных (видео, аудио, ...)
  - позволяет выстраивать p2p отношения между двумя браузерами
  - открытый код
  - на протокольном уровне ietf описывается двумя протоколами RTSP (real time streamin protocol для управления серверным потоком, прикладной уровень) и RTP (real time protocol, прикладной уровен, нужен для потоковой передачи, пакеты: временная метка, кодировка, формат полезной нагрузки, аглоритм скрытия ошибок, динамический порт, работает над UDP)
  - (очень просто): есть два браузера, расположенных в разных географических местах, и нужно передать видео с одного на другой
  - Обязательное условие - белые ip-адреса
  - можно делать на JS, есть апи, можно использовать
  - RealTimeControlProtocol: измеряются метрики качества, передаваемого потока

IndexDB API ДОПИСАТЬ  

15.5.24  

- есть направление браузерного программирования (разработка приложений на основе браузера)
- браузер устроен модульно и позволяет практически... каждый модуль живёт по отдельности и разрабатывается разными компаниями
- можно замаскировать браузерное приложение так, что браузера не будет видно вообще
- есть javascript engine: контейнер, в котором выполняется жабаскрипт
- нетворкинг занимается взаимодействием с сетью (запросы, вебсокеты)

22.05.2024  
ECMA - европейская комиссия по стандартизации в электронике  
ECMAScript - JS стандарта этой организации, он зарегистрован в ISO  
Программирование в Интернет - разработка серверного приложения, раюотающего по протоколу, описанному в неоктором RFC (можно даже придумать свой протокол и зарегистрировать его в IETF)  
Самое важное в веб-программировании - понимать ХТТП протокол  
3 основных селектора DOM:
- по айди
- по классу
- по тегу
AJAX - методология. Технологию надо разработать, методология уже есть готовая - бери и делай  
