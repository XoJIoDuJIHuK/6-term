...
30.04.2024  
WebAPI - реализация REST-серверов, REST - один из архитектур клиент-серверного приложения, связанная с появлением SPA, когда за сч ёт богатого клиента упрощается сервер (не гонит разметку, только возвращает джейсоны или XMLины), использует в основном 4 метода: (get, post, put, dlete), есть правила, сводящиеся к архитектуре клиент-сервре, отсутствию серверного состояния, кеширование только на стороне клиента и клиент явно управляет кешированияем, единообразный интерфейс, для клинта сервере представляется конечным (работать только с один сервером и ничего не знать кроме тех uri, которые были ему предоставлены), допускается загрузка апплетов  
