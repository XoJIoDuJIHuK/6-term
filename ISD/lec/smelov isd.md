...
Является частью IOC (Inversion of Control) (всё, что выполняется и имеет алгоритмическую основу, надо выносить в отдельный класс и передавать в нужный класс, например, через конструктор)  

19.03.2024  
Моделями называются 3 разных понятия:  

- в MVC (модель данных, внешний их источник)
- набор параметров, поступающих в действие контроллера
- данные, котороы можно передать в качестве параметра в представление

## Представления  

Центрьный кмпонент представления - razor engine, который обрабатывает представления, превращает их в объекты C# и формирует ответ  
@model означает, что данные, переданные в представление, имеют тип, следующий за данной директивой
@Model - это обращение к публичным свойствам представления (объекта представления) (например, @Model.a образается с свойству a модели)
Даже если параметры передаются через конструктор представления, они сами запихиваются во ViewBag (*точнее, записываются в свойство ViewBag.Model*)  
Коллекции

- ViewBag
- ViewData
- TempData
те же яйца, только в профиль (иногда бывает удобнее использовать индексатор вместо обращения к свойству через точку (ViewData["A"] вместо ViewBag.A))  
Model = ViewBag.Model = ViewData["Model"]  

@inherits - директива, позволяющая создать собственный базовый класс для представления (тогда представление будет производным от указанного класса, но класс должен наследоваться от RazorPage/RazorPage\<ModelType\>)  

26.03.2024  
Модель - объект, который может быть передан в представление  
Вспомогательные методы - такие объекты/методы/компоненты, которые позволяют создавать раметку; методы расширения специального дженерик-класса IHtmlHelper (вроде по имени интерфейс, но это класс)  
Также есть IUriHelper. В рамках MVC-приложения вся маршрутизация осуществляется по модели маршрутизатор-контроллер-представление. Хелпер позволяет генерировать правильные urlы на основе тех параметров, которые мы сюда (?) передаём  
TagHelper - один инструмент, чтобы править всеми. Позволяет создавать собственные теги, которые потом будут транслироваться в закреплённую за ними разметку  
Тег cache кеширует свои внутренности на стороне сервера (например, для затратных вычислений/большой разметки)  

16.04.2024  
Разделение компонент на зависящие и не зависящие от СУБД компоненты (провайдеры)  
У entity-объекта есть идентификатор, данные, описывающие свойства этого объекта и он содержит в себе метод(?)  
В ООП мы манипулируем entity-объектами  
Структура не зависит от применения данных, т.е. природа данных существует сама по себе  
В репоизтории не должно быть круда, должна быть бизнес-логика  
Интерфейс предполагает несколько реализаций. Значит, интерфейс репозитория предполагает наличие нескольких разных репозиториев  
ORM - таблице обычно соответствует класс с такой же структурой  

23.04.2024  
Стараются разделять уровень доступа к данным и уровень бизнес-логики  
ORM - суть база данных + ООП, и это абстракция над данными, реализованная средствами ООП  
Code-first, Database-first, Model-first  

30.04.2024  
WebAPI - реализация REST-серверов, REST - один из архитектур клиент-серверного приложения, связанная с появлением SPA, когда за счёт богатого клиента упрощается сервер (не гонит разметку, только возвращает джейсоны или XMLины), использует в основном 4 метода: (get, post, put, dlete), есть правила, сводящиеся к архитектуре клиент-сервре, отсутствию серверного состояния, кеширование только на стороне клиента и клиент явно управляет кешированияем, единообразный интерфейс, для клинта сервере представляется конечным (работать только с один сервером и ничего не знать кроме тех uri, которые были ему предоставлены), допускается загрузка апплетов  

Есть куча технологий, в каждой из которых это делается по-своему. Самый просто способ в аспах - handler. Возвращаемые значения сериализуются в json, реже в xml  
В некоторых случаях REST-сервер совмещают с сервером, отдающим разметку (когда нет смысла отдельно ставить нжинкс)  
Существуют разные подходы при генерации ответов. Есть лаконичный (на ГЕТ надо отвечать json/xml. Нужно ли что-то вовзрашать на оставльные методы? При таком подходе тело не возвращается, возвразается только ответ со статусом (200 или 400, не важно). Часто бывает неудобно (пусть есть список элементов на странице. Тогда удобнее при отправке запроса на удаление на сервер и получении кода 200 и идентификатора удалённого объекта удалить этот объект со страницы))
Все идентификаторы стоит выносить в URI  
Если поддерживается несколько коллекций, то удобнее писать прилодение при помощи MVC, где каждый контроллер обрабатывает свою коллекцию  
Хороший сервис позволяет указывать только те атрибуты, которые изменились, чтобы не выполнять лишнюю работу и не тягать большие объекты, когда не надо  

## Middleware

Есть картинка в лекции с порядком мидлвар. Порядок важен, потому что выход одних должен поступать на вход других

07.05.2024  

## Краткий обзор курса

- Есть платформа асп дотнет кор, которая в свою очередь работает на платформе дотнет кор (платформа, являющаяся развитием дотнет фреймворка)
  - дотнет кор
    - главная особенность - кроссплатформенность за счёт существования нескольких версий платформы для разных ОС
    - состоит из Core CLR и Core FX (эти - платформозависимые, есть для винды, линукса и макоси)
  - асп дотнет кор
    - унаследовала кроссплатформенность
    - открытый код (поддерживается сообществом)
    - предназначена для разработки веб-приложений
    - всё поддерживается через нугет-пакеты (плата за кроссплатформенность, теперь поддерживаются зависимости на уровне пакетов)
    - все приложения имеют одинаковую структуру
    - есть встроенный dependency injection (штатная возможность асп дотнет кор)
    - есть CLI, с помощью которого можно делать всякое
    - запуск любого приложения осуществляется через команду dotnet (*хотя, по-моему, это просто обращение к dotnet.exe*)
    - поддерживается Visual Studio
    - в основе лежит интерфейс OWIN (соединяет 4 части: хост (dotnet), сервер (kestrel), приложение (может быть либо конечной точкой, либо маршрутизатором), мидлваре)
      - в первых версиях асп дотнет кор это было явно, был файл Startup и такой же класс, впоследствии они были спрятаны под билдером
    - многие компоненты асп дотнет кор реализованы как мидлваре
    - структура проекта
      - существует папка wwwroot, предназначенная для хранения статических ресурсов (UseStatic)
    - простейшее приложение (обработчик)
    - любое приложение сводится к одному обработчику, получающему HttpContext (хз, что имелось в виду)
    - MVC
      - (картинка с приложением, ресурсом (программный код) и объектами запроса и ответа)
      - как программисты, можем разрабатывать приложение, мидлваре и dependency injection
      - такой стандартный интерфейс нужен для того, чтобы отдельные разработчики могли пистать сервера, другие - мидлваре, третьи - хосты
      - данный стандарт позволил разработчикам создавать фреймворки
      - (схема с клиентом, маршрутизатором, контроллером и представлением)
      - опять про фабрику контроллеров
      - контроллер создаётся на каждый запрос (!)
      - IActionResult: раньше в дотнет кор придерживались принципа "всё, что начинается с I, является интерфейсом", но в последнее время от этго отходят, поэтому непонятно, класс это или интерфейс
      - dependency injection - в самом простейшем случае механизм, позволяющий описать некоторый объект, который будет потом создаваться автоматически (без new) и мы можем указать его жизненный цикл (рассматривали transient (создаётся при каждом упоминании) и singleton (создаётся один раз на запрос), есть ещё scope). Позволяет структурировать
      - параметры в запросе сопоставляются с параметрами в экшене по именам
      - параметры можно упаковывать в различные классы

21.05.2024  
9:00, заходят 8 человек  
взять все лабы (если человек молчит, а в лабе это есть)  
для того, чтобы получить задачу, надо его сильно разозлить  

что такое дотнет, чем дотнет отличается от дотнет кор  
паттерн MVC находится в мидлваре, а код мы пишем в application  
ORM - паттерн проектирования (важно знать, что соответствует БД (dbcontext), таблице, строке (объект в dbset), структуре таблицы (модель) и т.д.)  
Identity: находится в мидлваре (а мы используем атрибуты в application)  
WebAPI: чем REST оличается от MVC, что такое HATEOS  
